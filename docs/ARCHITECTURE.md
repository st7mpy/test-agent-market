...
**Consequences:** someone must operate the flag/override monitor (active-dispute feeds, subjective-criteria review); the weights are policy and need calibration against realized dispute outcomes. The **depositor insurance fund** (the other half of C — a backstop funded by protocol fee + slashing proceeds, paid out on oracle-failure losses) is the heavier vault-side follow-on, not in this pre-trade control.
