...
### ADR-014 — Oracle/resolution risk is priced and gated pre-trade; exposure-increasing only (differentiator C)
**Choice:** an `OracleRiskModel` (`predmkt/oracle_risk.py`) scores each market's oracle/resolution risk in [0,1] (category base + monitor-set flags, with overrides); the Risk Gate **refuses new exposure** above a cap and **scales the notional cap by `(1 − score)`** below it. Gating applies **only to exposure-increasing intents** — a market that turns risky can always be exited.
**Rationale:** the payout depends on the oracle (UMA) resolving correctly, which has failed in production (~$7M falsely settled via concentrated voting power). It's the #1 systemic risk and *no competitor prices it*. Putting the control at the existing Risk Gate choke point (ADR-002) means every order is checked once; sizing-down (not just binary refuse) lets the system keep trading low-risk markets normally while throttling dangerous ones. The increase-only rule is the critical safety property: blocking *all* orders in a risky market would trap capital — you must be able to flatten. Scores are transparent/auditable because depositors are told what risk they carry, and because "ambiguous criteria" can't be read off-chain — a monitor flags it, the model just bounds it.
**Rejected:** *ignore oracle risk* (the status-quo competitor stance — uninsured tail risk); *binary allow/deny only* (too blunt — kills good markets or over-permits); *gate all orders incl. exits* (traps capital in exactly the markets you must leave); *resolve "ambiguity" from market data* (not observable — needs an explicit monitor flag).
**Consequences:** someone must operate the flag/override monitor (active-dispute feeds, subjective-criteria review); the weights are policy and need calibration against realized dispute outcomes. The **depositor insurance fund** (the other half of C) is implemented vault-side per ADR-015.

### ADR-015 — Realized-loss waterfalls + depositor insurance fund, by loss attribution (differentiator C)
**Choice:** the vault carries an **insurance fund** (reserved USDC, excluded from depositor NAV, funded by `fundInsurance` + `slashToInsurance`), and two RiskGate-only loss functions split by *who is at fault*: `applyStrategyLoss` (a maker trading loss) draws **first-loss → bond → depositors**; `coverOracleLoss` (an oracle failure) draws **insurance fund → depositors**, leaving maker capital untouched. Each just reduces the right reserved bucket, so `totalAssets = balance − reserved` keeps depositors whole up to that bucket's size; the residual is what they bear.
**Rationale:** loss attribution should drive who pays. A *strategy* loss is the maker's responsibility, so their junior first-loss absorbs it first (ADR-005) — that's the alignment the whole bond/first-loss design exists for. An *oracle failure* is **not** the maker's fault (and is gated, never eliminated, by ADR-014), so raiding the maker's alignment capital for it would be unfair and would punish makers for a venue risk; instead the protocol-funded insurance fund eats it, which is exactly the marketed differentiator ("the only vault that protects you from oracle failure"). Implementing both as bucket-draws reuses the same accounting as bond/first-loss exclusion, so depositor-NAV correctness falls out of one invariant rather than bespoke math. Note the asymmetry with `slash` (bond→first-loss): slashing is a *penalty* (bond is the penalty buffer), loss-absorption is *junior-tranche-first* (first-loss is the designated absorber) — different purposes, different order, both intentional.
**Rejected:** *one undifferentiated loss path* (makes makers eat oracle risk or depositors eat strategy risk — both misattribute); *insurance fund inside NAV* (would inflate share price and pay the backstop out as phantom yield); *fund auto-replenished from the fee inside `_crystallizeFee`* (entangles fee math with funding — kept separate via explicit `fundInsurance` for clean accounting and auditability).
**Consequences:** the loss amount is a **trusted RiskGate input** (the authoritative reconciliation, ADR-011), so that measurement path is in the audit scope; the insurance fund must be *sized* against the oracle-risk exposure it backstops (a policy/actuarial question — beyond fund size, depositors still bear the tail); fund accounting is one more reserved bucket to reconcile.

### ADR-016 — Multi-venue behind one adapter seam; venue-aware fees; resolution-match is asserted (differentiator D)
**Choice:** every venue is a `VenueAdapter` implementation (`PolymarketAdapter`, `KalshiAdapter`, `ReplayAdapter`); a `CrossVenueFeed` aggregates two adapters for the same event into a `CrossVenuePair`; fees are computed **per venue** (`taker_fee_per_share` dispatches Polymarket's `min(p,1-p)`+cap vs Kalshi's `0.07·p(1-p)`); and `resolution_rules_match` is an **asserted** flag that **defaults to False** (treat as basis risk until a human confirms identical settlement).
**Rationale:** (a) the venues are a strategic risk — Polymarket owns >96% of volume and the API the whole ecosystem depends on (COMPETITION §4) — so no single venue can be load-bearing; one seam makes adding/dropping venues cheap. (b) Multi-venue lifts the OI-bound capacity ceiling (the same edge has more room across venues) and unlocks *real* cross-venue arbitrage (DIFFERENTIATION D). (c) The same fill costs **different** fees on each venue; charging Polymarket's fee on a Kalshi leg mis-states the edge and would mis-trigger arb — so the edge must be priced venue-by-venue. (d) You **cannot** auto-verify two venues resolve on identical criteria; assuming they match is the classic prediction-market trap that turns "arbitrage" into an unhedged directional bet, so the safe default is basis-risk and matching must be explicitly asserted.
**Rejected:** *single-venue* (platform/API risk); *one fee model for all venues* (mis-prices cross-venue edge); *auto-assume resolution rules match* (basis-risk landmine); *build venue connectivity from scratch* (wrap OSS aggregators — pmxt / agg.market / PolyRouter — behind the adapter instead).
**Consequences:** each venue needs its own adapter + fee model + a human-maintained resolution-rules mapping; the live adapters stay network-gated/untested in-session; custody differs per venue (ADR-008) — the **Kalshi rail's regulated custody keeps it Phase-8-gated**, even though its market-data adapter exists now.

### ADR-017 — Tranched depositor capital: senior (subordination-protected) + junior (first-loss, upside) (differentiator E)
**Choice:** `TranchedVault` splits *depositor* capital into a **senior** tranche (loss-subordinated, with a capped yield-priority coupon) and a **junior** tranche (absorbs losses first, takes the residual yield). Losses draw junior→senior; yield pays the senior coupon→junior residual; both are controller-only on the authoritative accounting, preserving the invariant `assetBalance == seniorAssets + juniorAssets`.
**Rationale:** one flat risk profile fits no one — it's too risky for conservative capital and too diluted for alpha-seekers. Tranching widens the funnel: conservative money buys the senior (steady, principal-protected *by subordination*, funded from the market-neutral venue-yield of B), while risk-on money buys the junior and is *paid* for bearing first loss via levered upside on the same gross return. It's the same loss-waterfall philosophy as ADR-015 (junior-first), now generalized from the maker's first-loss to the depositor side — so the moat's "principal-protected prediction-market yield" product is real, not marketing. Controller-only, accounting-driven yield/loss mirrors the rest of the system (trusted reconciliation, ADR-011).
**Rejected:** *single flat tranche* (one-size-fits-none); *two full ERC4626 vaults + a coordinator* (more surface than a skeleton needs — internal share ledgers show the mechanic); *mutable coupon* (unfair to existing holders → fixed at issuance / immutable); *calling senior "principal-protected" unqualified* (it's subordination — senior still bears loss once the junior buffer is exhausted, and the tests prove exactly that).
**Consequences:** tranche shares are non-transferable internal balances in the skeleton (production: an ERC20/ERC4626 per tranche); first-depositor share-inflation hardening and per-tranche fee/HWM are out of scope; the controller's yield/loss inputs are trusted (audit scope, as in ADR-015); senior-coupon sizing vs the expected junior buffer is a structuring/actuarial question.

### ADR-018 — TEE-private positions + attested track records; A-lite, not A-full (differentiator A)
**Choice:** run the maker strategy inside a **TEE** (Phala/Oasis) so live positions are private from the public/copycats while the enclave emits an **attested** verified track record (code measurement + results + a hiding commitment to positions, with a remote-attestation quote). Commit to **A-lite**: the platform still sees positions (the RiskGate + key-holding OMS need them); only outsiders are blind. Full design + the attestation-flow stub: `docs/TEE-DESIGN.md`, `strategies/predmkt/attestation.py`.
**Rationale:** every competitor's positions are forced on-chain, which is exactly what lets copy-bots decay the alpha the vault sells; ADR-009's delay lever is weak (too short leaks, too long isn't credible). A TEE resolves it cryptographically — "trade with proof, not trust": verifiable performance *without* revealing trades, so copy-bots can't copy what they can't see. The TEE *is* the Phase-5 sandbox (it already has to isolate untrusted code), so privacy + attestation come on top of the same trust boundary (ADR-002), not as a new system. A-lite is chosen over A-full because keeping keys + RiskGate outside the enclave preserves the existing custody (ADR-008) and reconciliation (ADR-011) designs and is shippable solo; A-full (platform also blind) reworks both and is a research track.
**Rejected:** *full real-time public positions* (copy-decay — the competitors' forced weakness); *delay-only obfuscation* (ADR-009 alone — a weak lever); *ZK proofs of performance instead of a TEE* (heavier/less mature for live trading today; revisit); *A-full now* (custody/reconciliation rework, too heavy for Phase 5).
**Consequences:** real enclaves need Phala/Oasis infra + network (not stood up here — the deliverable is the design + a stdlib-HMAC attestation-flow stub); the Phase-5 **security gate (sandbox-escape + key-isolation pentest, Audit #2) still applies** — a TEE is a strong boundary, not a substitute, and side-channel/supply-chain risks must be threat-modeled; positions are revealed on a lag and checked against the commitment (ADR-009 delayed disclosure).
